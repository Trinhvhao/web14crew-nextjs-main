...
const projectConfig: Record<ProjectKind, {
  ...
  corp: {
    ...
    bigTitleKey: "detail.corp.bigTitle",
    bg: "#791220",
    fg: "#f4efe6",
    panel: "#d31c36",
    gridImages: [
  ...
  prod: {
    ...
    bigTitleKey: "detail.prod.bigTitle",
    bg: "#791220",
    fg: "#f4efe6",
    panel: "#d31c36",
    gridImages: [
...
export default function ProjectDetailPage({ kind, slug }: { kind: ProjectKind; slug: string }) {
  ...
  const bigTitle = t(config.bigTitleKey);
  ...
  if (!isTvc) {
    const gridImages = config.gridImages ?? remainingItems.map((item) => item.img);
    const btsImages = config.btsImages ?? [];
    const crewInfo = t("detail.common.crewInfo");
    return (
      <section
        ref={sectionRef}
      ...
      >
        ...
        <div className="absolute left-8 bottom-24 flex flex-col items-center gap-[6px]">
          <span
            className="text-[13px] font-bold uppercase tracking-[0.15em] font-sans mb-[8px]"
            style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", color: "#F2C94C" }}
          >
        ...
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 flex flex-col md:flex-row h-full gap-6 md:gap-12 pt-8 md:pt-14">
          <div className="w-full md:w-[35%] flex flex-col h-full shrink-0">
            <div>
              <h2 className="text-base sm:text-xl md:text-[2rem] font-bold uppercase tracking-tight border-b-[3px] inline-block pb-1 mb-2 w-fit leading-none" style={{ borderColor: config.fg }}>
                {t("detail.common.producedBy")}
              </h2>
            ...
            <div className="mb-auto mt-8">
              <div className="flex items-center w-full whitespace-nowrap">
                ...
                <div className="flex-grow h-[2px] relative ml-2 -mt-4 opacity-80 min-w-[50px]" style={{ backgroundColor: config.fg }}>
                  <div className="absolute -right-1 top-1/2 -translate-y-1/2 w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-l-[8px]" style={{ borderLeftColor: config.fg }} />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 w-full mb-4 relative">
              <div className="absolute -top-6 left-0 right-0 flex items-center justify-between">
                <div className="w-full h-[2px] relative" style={{ backgroundColor: config.fg }}>
                  <div className="absolute -left-1 -top-[4px] w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-r-[8px]" style={{ borderRightColor: config.fg }} />
                  <div className="absolute -right-1 -top-[4px] w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-l-[8px]" style={{ borderLeftColor: config.fg }} />
                </div>
              </div>
              {gridImages.map((src, i) => (
                <div
                  key={`${src}-${i}`}
                  className="aspect-video relative overflow-hidden cursor-pointer group shadow-md"
                  style={{ backgroundColor: "#9ccc65" }}
                >
                  <Image src={src} alt={`${sectionName} thumbnail ${i + 1}`} fill sizes="180px" className="object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                </div>
              ))}
            </div>
          </div>

          <div className="w-full md:w-[65%] flex flex-col h-full md:border-l-[1px] md:pl-12 py-2" style={{ borderColor: `${config.fg}30` }}>
            {btsImages.length > 0 && (
              <div className="flex justify-center gap-2 md:gap-6 mb-4 flex-wrap">
                {btsImages.map((src, i) => (
                  ...
                  >
                    <Image src={src} alt={`${sectionName} BTS ${i + 1}`} fill sizes="180px" className="object-cover" />
                  </div>
                ))}
              </div>
            )}
            ...
            >
              <Image
                ...
                fill
                priority
                sizes="(max-width: 1440px) 100vw, 936px"
