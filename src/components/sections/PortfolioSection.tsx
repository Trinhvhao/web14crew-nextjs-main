...
export default function PortfolioSection({
  ...
  const marqueeInView = useInView(marqueeRef, { amount: 0.25 });

  useEffect(() => {
    if (!autoRotate || items.length <= 1 || hovered || !inView) return;
    const timer = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % items.length);
    }, rotateInterval);
    return () => clearInterval(timer);
  }, [autoRotate, items.length, rotateInterval, hovered, inView]);
  ...
  return (
    <section ref={sectionRef} data-scroll-id={id} className="relative flex flex-col pt-16 md:pt-8 pb-3 px-4 md:px-8 z-10 w-full bg-[#f4efe6] text-black h-screen md:h-screen font-sans overflow-hidden shrink-0">
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 flex flex-col h-full">
        ...
        <FadeIn direction="up" delay={0.08} className="relative w-full shadow-lg bg-white p-2 pb-0 flex-1 min-h-0 group">
          {mainHref ? (
            <Link
              href={mainHref}
            ...
            >
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <Image src={mainImage} alt={mainTitle} fill sizes="(max-width: 768px) 100vw, 1200px" quality={70} className="object-cover object-center" />
                </motion.div>
              </AnimatePresence>
              {!hovered && (
                <motion.div
          ...
          <FadeIn direction="up" delay={0.12} className="mt-4 md:mt-8 overflow-hidden w-full pb-2 flex-shrink-0 h-[80px] sm:h-[100px] md:h-[120px]">
          <div ref={marqueeRef} className="animate-marquee gap-5 h-full" style={{ animationPlayState: marqueeInView ? "running" : "paused" }}>
            {[...items, ...items].map((item, index) => (
              <Link
                key={`${item.id}-${index}`}
                href={`/${routeBase}/${item.slug}`}
                data-cursor-text="OPEN"
                className="relative w-[220px] h-full flex-shrink-0 group cursor-pointer overflow-hidden bg-gray-900 border border-transparent hover:border-brand-yellow transition-all block"
              >
                <Image src={item.img} fill sizes="220px" quality={65} className="object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100" alt={item.name} />
        ...
        <FadeIn direction="up" delay={0.18} className="flex justify-center mt-4 flex-shrink-0">
