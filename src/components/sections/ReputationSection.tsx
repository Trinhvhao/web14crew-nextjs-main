...
export default function ReputationSection() {
  ...
  return (
    <section className="relative flex flex-col pt-8 pb-6 px-4 md:px-8 z-10 w-full bg-[#791220] text-[#f4efe6] h-screen font-sans overflow-hidden shrink-0">
      <div className="absolute inset-0 z-0 opacity-20 mix-blend-overlay pointer-events-none flex items-center justify-center"><Image src="https://images.unsplash.com/photo-1596704017254-9b121068fb31?auto=format&fit=crop&q=80&w=2000&h=1500" alt="Background texture" fill sizes="100vw" loading="lazy" placeholder="blur" blurDataURL="data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxIDEiLz4=" className="object-cover filter blur-[2px] contrast-125" /></div>
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 flex flex-col h-full z-10 relative">
        <FadeIn direction="down" delay={0.2}><h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-[3rem] xl:text-[5rem] font-display font-normal uppercase tracking-tighter leading-[0.8] transform scale-y-[1.3] scale-x-[0.95] origin-left mb-4 md:mb-6 text-[#f4efe6] mt-4">{t("reputation.title")}</h2></FadeIn>
